...
(function(){
	L.FormUtility = (function(){
	var CslEngine = L;
	var container_locators = {};
	...
	CslEngine.Plugin.register({name:"loadStyle.formUtility", fn: (function formUtilityStyleTokensSave(object, callback){
		styleTokens = object;
		console.log(arguments, styleTokens)
		return callback(object);
	}), post:"loadStyle.parseData"});
	CslEngine.Plugin.register({name:"loadLocale.formUtility", fn: (function formUtilityLocaleTokensSave(object, callback){
		localeTokens = object;
		return callback(object);
	}), post:"loadLocale.parseData"});
	
	var getMonthLabels = function(){
		return monthValues.map(function(value){
			return CslEngine.Private.Locale.getTerm(value,{form:"long"});
		})
	}
	var getSeasonLabels = function(){
		return seasonValues.map(function(value){
			return CslEngine.Private.Locale.getTerm(value);
		})
	}
...
//		console.log(citeproc.cslXml.dataObj.children);
		var interface_node = styleTokens.children.filter(function(item){ return item.name == "interface"});
