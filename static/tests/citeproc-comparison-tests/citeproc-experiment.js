...
CSL.setDecorations = function (state, attributes) {
	//// Text decorations currently handled separatly from normal attributes.
    var ret, key, pos;
    ret = {}; /// Must be dict, not an array. This will prevent output like <sup><sub>STRING</sub></sup>.
...
CSL.Node.bibliography = {
    build: function (token, buildObj) {
//		var state=buildObj;
        if (token.tokentype === CSL.START) {
			if (!buildObj["bibliography"]){
				buildObj["bibliography"] = {tokens:[],opt:{},sort:{}};
			}
			buildObj.tokens = buildObj["bibliography"].tokens;//this.Engine.bibliography.tokens;
...
CSL.Node.date = {
	build: function(token, buildObj, scopeObj){
		if (token.tokentype == CSL.END){
			token.children = {};
			scopeObj.children.map(function(child){
				/// child is an array of [open, close] or [singleton] --> first token has attributes.
				if (child[0].name == "date-part"){
					token.children[child[0].strings.name] = child[0];
				}
			})
			if (buildObj.area == "locale"){
				var form = scopeObj.open.strings.form || "text";
				this.Locale.setDateFormat(form, token, buildObj.lang)
			}
		}
...
CSL.Node["date-part"] = {
    build: function (token, buildObj, scopeObj) {
		if (buildObj["area"] == "locale"){return}
...
CSL.Node["date-part"] = {
	build: function(token, buildObj, scopeObj){
//		scopeObj.parent[token.strings.name] = token;
	}
}
...
CSL.Node.locale = {
    build: function (token, buildObj, scopeObj) {
        if (token.tokentype === CSL.START) {
			buildObj.tokens = [];
			buildObj.area = "locales";
			buildObj.lang = token.lang;
		} else if (token.tokentype === CSL.END) {
...
//			this.Engine["locales"][token.lang].opt = buildObj.opt;
			buildObj.target = [];
			buildObj.area = ""
		}
    }
...
/*                if (this.strings.label_form_override) {
                ...
                if (this.realGroup) {
                    ...
                    }/* else if (this.strings.reject) { ///reject & require appear unused.
                        condition = {
...
CSL.Node["if"] = {
    build: function (token, buildObj, scopeObj) {
		...
		} else if (token.tokentype == CSL.SINGLETON){
		    buildObj.tokens.push(token);
			token.test = this.Match.merge(token.tests, token.match);
			token.fail = buildObj.tokens.length;
		/// if Tok fails, go to next tok
		/// if Tok succeeds, we must skip all following tests.
		}
    }
};
CSL.Node["else-if"] = CSL.Node["if"];

CSL.Node["else"] = {
    build: function(token, buildObj) {
		token.succeed = buildObj.target.length;
//        buildObj.tokens.push(token); /// not strictly necessary.
    },
...
CSL.Node["condition"] = {
    build: function (token, buildObj, scopeObj) {
        if ((token.tokentype === CSL.SINGLETON) || (token.tokentype === CSL.START)) {
...
CSL.Conditions = {};
CSL.Conditions.TopNode = function (token, buildObj) {
	var fun = CSL.Engine.Fun({})
    var func;
    if (this.tokentype === CSL.START || this.tokentype === CSL.SINGLETON) {
        if (this.locale) {
        ...
        if (!this.tests || !this.tests.length) {
            ...
            this.test = this.Match[token.match](token, token.tests);
        }
    }
    if (this.tokentype === CSL.END || this.tokentype === CSL.SINGLETON) {
        func = function (state, Item) {
...
CSL.Conditions.Engine.prototype.matchCombine = function () {
    this.token.test = Match[this.token.match](this.token, this.state, this.token.tests);
};
CSL.Node.info = {
    build: function (state, target) {
        if (this.tokentype === CSL.START) {
...
CSL.Node.key = {
    build: function (token, buildObj) {
        var func, i, ilen;
		var self = this;
		var area = buildObj.area;
        var start_key = new CSL.Token("key", CSL.START);
		console.log(start_key, buildObj);
        start_key.strings["et-al-min"] = token.strings["et-al-min"];
        start_key.strings["et-al-use-first"] = token.strings["et-al-use-first"];
        start_key.strings["et-al-use-last"] = token.strings["et-al-use-last"];
        func = function (token, state, Item) {
            state.tmp.done_vars = [];
//			execObj.sort["done_vars"] = []
        };
...
/*        func = function (state, Item) {
        ...
        start_key.execs.push(func);*/
        var sort_direction = [];
        if (token.strings.sort_direction === CSL.DESCENDING) {
        ...
        this.Engine[buildObj.area].sort.opt.sort_directions.push(sort_direction);
        if (CSL.DATE_VARIABLES.indexOf(token.variables[0]) > -1) {
            state.sort.date_key = true;
        }
        func = function (token, state, Item) {
            state.tmp.sort_key_flag = true;
            if (token.strings["et-al-min"]) {
                state.tmp["et-al-min"] = token.strings["et-al-min"];
            }
            if (token.strings["et-al-use-first"]) {
                state.tmp["et-al-use-first"] = token.strings["et-al-use-first"];
            }
            if ("boolean" === typeof token.strings["et-al-use-last"]) {
                state.tmp["et-al-use-last"] = token.strings["et-al-use-last"];
            }
        };
        start_key.execs.push(func);
        buildObj.target.push(start_key);
        if (token.variables.length) { //// has variables attribute --> is not a macro
            ...
            if (CSL.CREATORS.indexOf(variable) > -1) {
                ...
                buildObj.target.push(single_text);
            }
        } else { // macro /// call to macro (not implemented yet)
...
//            CSL.expandMacro.call(state, token, target);
        ...
        if (state.sort.date_key) {
            ...
            state.sort.date_key = false;
        }
/*        func = function (token, state, Item, execObj) {
//            state.tmp.done_vars = [];
			execObj.sort["done_vars"] = []
            execObj.sort["et-al-min"] = undefined;
            execObj.sort["et-al-use-first"] = undefined;
            execObj.sort["et-al-use-last"] = undefined;
            execObj.sort.sort_key_flag = false;
        };*/
        func = function (token, state, Item) {
...
CSL.Node.macro = {
    build: function (token, buildObj) {
		if (token.tokenType == CSL.START){
//			var oldTarget = buildObj.target; //// create reference to original target
//			buildObj.target = buildObj.macros[macro.attr.name].tokens; //// change reference of target parameter
//			return oldTarget; //// return oldTarget to message the close tag.
		} else {
//			buildObj.target = buildObj.msg; //// revert from macro to original target area.
		}
	}
};
    ...
    build: function (state, target) {
        ...
        if ([CSL.SINGLETON, CSL.START].indexOf(this.tokentype) > -1) {
            ...
            func = function (state, Item) {
                ...
                this.and = {};
                if (this.strings.and) {
                    state.output.append("j", this.and_term, "empty", true);
                    this.and.single = state.output.pop();
                    this.and.single.strings.prefix = this.and_prefix_single;
                    this.and.single.strings.suffix = this.and_suffix;
                    state.output.append(this.and_term, "empty", true);
...
CSL.Node.names = {
	build: function(token, buildObj){
		token.variables.map(function(variable){
			if (this.Style.nameVars.indexOf(variable) == -1){
				this.Style.nameVars.push(variable); //// required to extract names from added Source Items
			}
		})
	}
}
...
CSL.Node.sort = {
    build: function (token, buildObj) {
        if (token.tokentype === CSL.START) {
			this.Engine[buildObj.area]["sort"] = {tokens:[],opt:{sort_directions:[]}};
	        target = this.Engine[buildObj.area].sort.tokens;
            var func = function (state, Item) {
            ...
            token.execs.push(func);
        }
        if (token.tokentype === CSL.END) {
			target = this.Engine[buildObj.context[1]].tokens;
            var func = function (state, Item) {
                if (state.opt.has_layout_locale) {
...
CSL.Node.number = {
    build: function (token, buildObj) {
		if ((token.tokentype==CSL.END)||(token.tokentype==CSL.SINGLETON)){
			var self = this;
		    if (token.strings.form === "roman") {
		        token.formatter = this.Engine.fun.romanizer;
		    } else if (token.strings.form === "ordinal") {
		        token.formatter = this.Engine.fun.ordinalizer;
		    } else if (token.strings.form === "long-ordinal") {
		        token.formatter = this.Engine.fun.long_ordinalizer;
		    }
			function outputNumber(token, Item, context, ExecObj){
		        if (token.variables.length === 0) {
		            return ;
		        }
				ExecObj.target.push(NumberBlob.call(self, token, Item));
		    };
		    token.execs.push(outputNumber);
		    buildObj.target.push(token);
		}
    }
};
...
CSL.Attributes["@punctuation-in-quote"] = function (token, buildObj, arg) { //// locale
	token.strings["punctuation-in-quote"] = arg;
};

CSL.Attributes["@limit-day-ordinals-to-day-1"] = function (token, buildObj, arg) { //// locale
	token.strings["limit-day-ordinals-to-day-1"] = arg;
};
CSL.Attributes["@xml:lang"] = function (token, buildObj, arg) { //// locale
	token.lang = arg;
	if (token.tokentype==CSL.START){
//		this.Locale.initializeLanguage(arg);
	}
};

CSL.Attributes["@type"] = function (token, buildObj, arg) {
    var types = arg.split(/\s+/);
    var maketest = function (mytype) {
//		console.log("maketest", mytype, token);
        return function(Item,item) {
//			console.log("test",mytype, Item.type)
...
//	console.log("tokenTests", token.tests)
};
CSL.Attributes["@variable"] = function (token, buildObj, arg) {
...
//    this.variables_real = this.variables.slice();
    if ("label" === this.name && this.variables[0]) {
        ...
        function setTokenVariable (token, Item, item, execObj) {
			if (["text", "number"].indexOf(token.name)){
				token.value = Item[token.variables[0]] || "";
			}
            var output = false;
			token.variables.map(function(variable){
                if (token.strings.form === "short" && !Item[variable]) {
                    if (variable === "title") {
				...
				if (varType == "names") {
                    if (Item[variable]) {
                        for (var key in Item[variable]) {
                            if (token.dateparts.indexOf(key) === -1 && "literal" !== key) {
                                //// pass
                            } else if (Item[variable][key]) {
                                output = true;
                            }
                        }
                    }
                } else if (["locator", "locator-extra","first-reference-note-number"].indexOf(variable) > -1) {
            ...
            if (output) {
				token.variables.map(function(variable){
                    if (variable !== "citation-number" || this.tmp.area !== "bibliography") {
//                        this.tmp.cite_renders_content = true;
                    }
//                    this.tmp.group_context.tip.variable_success = true;
...
//			console.log(token, token.name);
        ...
        token.execs.push(setTokenVariable);
    } else if (["if",  "else-if", "condition"].indexOf(token.name) > -1) {
...
CSL.Attributes["@page"] = function (state, arg) {
    ...
    for (var i=0,ilen=trylabels.length;i<ilen;i+=1) {
        token.tests.push(maketest(trylabels[i]));
    }
};
...
CSL.Attributes["@context"] = function (state, arg) {// test to see if in bib or cite build area
    var func = function (Item, item) {
...
CSL.Attributes["@gender"] = function (token, buildObj, arg) {
    token.strings.gender = arg;
}
CSL.Attributes["@gender-form"] = function (token, buildObj, arg) {
...
CSL.Attributes["@value"] = function (token, buildObj, arg) {
    token.strings.value = arg;
};
CSL.Attributes["@name"] = function (token, buildObj, arg) {
    token.strings.name = arg;
};
CSL.Attributes["@form"] = function (token, buildObj, arg) {
...
CSL.Attributes["@macro"] = function (token, buildObj, arg) {
    token.macro = arg;
};
CSL.Attributes["@term"] = function (token, buildObj, arg) {
    if (arg === "sub verbo") {
...
CSL.Attributes["@font-variant"] = function (token, buildObj, arg) {
    token.decorations["@font-variant"] = arg;
};
CSL.Attributes["@font-weight"] = function (token, buildObj, arg) {
    token.decorations["@font-weight"] = arg;
};
CSL.Attributes["@font-style"] = function (token, buildObj, arg) {
    token.decorations["@font-style"] = arg;
};
CSL.Attributes["@text-decoration"] = function (token, buildObj, arg) {
    token.decorations["@text-decoration"] = arg;
};
CSL.Attributes["@text-case"] = function (token, buildObj, arg) {
    token.decorations["@text-case"] = arg;
};
CSL.Attributes["@vertical-align"] = function (token, buildObj, arg) {
    token.decorations["@vertical-align"] = arg;
};
CSL.Attributes["@suffix"] = function (token, buildObj, arg) {
    token.strings.suffix = arg;
};
CSL.Attributes["@prefix"] = function (token, buildObj, arg) {
    token.strings.prefix = arg;
};
CSL.Attributes["@delimiter"] = function (token, buildObj, arg) {
/*    if ("name" == token.name) {
	    token.strings.name_delimiter = arg;
	} else {*/
        token.strings.delimiter = arg;
//    }
};
CSL.Attributes["@match"] = function (token, buildObj, arg) {
    token.match = arg;
};
...
CSL.Attributes["@name-delimiter"] = function (token, buildObj, arg) {
    buildObj.setOpt(token, "name-delimiter", arg);
};
CSL.Attributes["@et-al-min"] = function (token, buildObj, arg) {
	if (token.name == "names"){
		token.opt["et-al-min"] = arg;
	}
    var val = parseInt(arg, 10);
	buildObj.setOpt(token, "et-al-min", val);
};
CSL.Attributes["@et-al-use-first"] = function (token, buildObj, arg) {
    buildObj.setOpt(token, "et-al-use-first", parseInt(arg, 10));
};
CSL.Attributes["@et-al-use-last"] = function (token, buildObj, arg) {
    if (arg === "true") {
        buildObj.setOpt(token, "et-al-use-last", true);
    } else {
        buildObj.setOpt(token, "et-al-use-last", false);
    }
};
CSL.Attributes["@et-al-subsequent-min"] = function (token, buildObj, arg) {
    var val = parseInt(arg, 10);
/*    if (state[state.build.area].opt.max_number_of_names < val) {
        state[state.build.area].opt.max_number_of_names = val;
    }*/
    this.setOpt(token, "et-al-subsequent-min", val);
};
...
CSL.Attributes["@subsequent-author-substitute"] = function (state, arg) {
	state.setOpt(this, "subsequent-author-substitute", arg);
...
CSL.Attributes["@hanging-indent"] = function (state, arg) {
    state.opt.hangingindent = arg;
};
CSL.Attributes["@line-spacing"] = function (token, buildObj, arg) {
    if (arg && arg.match(/^[.0-9]+$/)) {
        buildObj.setOpt(token, "line-spacing", parseFloat(arg, 10));
    }
};
CSL.Attributes["@entry-spacing"] = function (token, buildObj, arg) {
    if (arg && arg.match(/^[.0-9]+$/)) {
		buildObj.setOpt(token, "entry-spacing", arg)
    }
};
...
CSL.Attributes["@page-range-format"] = function (token, buildObj, arg) {
