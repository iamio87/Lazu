...
//// TODO: look at XMl to Token to understand how to register tokens in citeproc.citation.tokens
...
CSL.ITERATION = 0;
CSL.Engine.prototype.tomsDateParse = function(dateArray){
	var ret = dateArray.map(function(dateObj){
		return ['year','month','day'].map(function(key, counter){
...
CSL.getCitationCluster = function (inputList, citationID) {
    ...
    for (pos = 0; pos < len; pos += 1) {
        ...
        item = CSL.parseLocator.call(this, item); /// Some doctoring of locator data.
                    ...
                    if (this.variables.length
						...
						if (date_obj){
							...
							if (date_obj.length && date_obj[0].hasOwnProperty && (typeof(date_obj[0])=="object")) {
								date_obj = state.tomsDateParse(date_obj)
							}
